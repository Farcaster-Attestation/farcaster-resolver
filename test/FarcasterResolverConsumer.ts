...
async function attestSimpleSchema(eas: any, consumer: `0x${string}`, alice: PrivateKeyAccount, fid: bigint) {
...
describe("StandardConsumer", function () {
  ...
  it("Basic Ref", async function () {
    const { eas, resolver, membership, schemaRegistry, alices, simpleConsumer } =
      await loadFixture(deploySimpleFixture);

    const uid = await attestSimpleSchema(eas, simpleConsumer.address, alices[0], 1n);
  ...
  it("Nested Ref", async function () {
    ...
    const uid2 = await attestSimpleSchema(eas, simpleConsumer.address, alices[0], 1n);

    const publicClient = await hre.viem.getPublicClient();

    const dummyEncodedData = encodeAbiParameters(parseAbiParameters("bytes32 dummy"), [
