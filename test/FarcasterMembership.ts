...
describe("FarcasterMembership", function () {
  it("Super admin can add and remove admin/member", async function () {
    ...
    expect(await membership.read.getMember([uid, 2n])).to.deep.equal([
      false,
      0n,
    ]);
    expect(await membership.read.getMember([uid, 1n])).to.deep.equal([
      false,
      0n,
    ]);

    expect(
      membership.write.setMember([uid, 3n, 2n, 0b1100000n], {
        account: alices[2],
      })
    ).to.not.be.rejectedWith("PermissionDenied()");

    await membership.write.setMember([uid, 1n, 2n, 0b1100000n], {
      account: alices[0],
    });
    ...
    await membership.write.setMember([uid, 2n, 3n, 0b1011111n], {
      account: alices[1],
    });
    await membership.write.setMember([uid, 1n, 4n, 0b0111111n], {
      account: alices[0],
    });
    ...
    expect(await membership.read.getMember([uid, 3n])).to.deep.equal([
      true,
      0b1011111n,
    ]);
    expect(await membership.read.getMember([uid, 4n])).to.deep.equal([
      true,
      0b0111111n,
    ]);

    await expect(
      membership.write.removeMember([uid, 3n, 1n], { account: alices[0] })
    ).to.be.rejectedWith("PermissionDenied");
    await expect(
      membership.write.removeMember([uid, 4n, 1n], { account: alices[3] })
    ).to.be.rejectedWith("PermissionDenied");
    await membership.write.removeMember([uid, 3n, 1n], { account: alices[2] });
    await expect(
      membership.write.removeMember([uid, 3n, 1n], { account: alices[2] })
    ).to.be.rejectedWith("PermissionDenied");
  ...
  it("Admin can add and remove member", async function () {
    ...
    await membership.write.setMember([uid, 1n, 2n, 0b1111111n], {
      account: alices[0],
    }); // Super admin
    await membership.write.setMember([uid, 2n, 3n, 0b0011111n], {
      account: alices[1],
    }); // Admin

    // Admin can add regular member
    await membership.write.setMember([uid, 3n, 4n, 0b0000111n], {
      account: alices[2],
    });
    expect(await membership.read.getMember([uid, 4n])).to.deep.equal([
      true,
  ...
  it("Admin can't add or remove super admin", async function () {
    ...
    await expect(
      membership.write.setMember([uid, 3n, 4n, 0b1111111n], {
        account: alices[2],
      })
    ).to.be.rejectedWith("PermissionDenied");
    ...
    await expect(
      membership.write.removeMember([uid, 3n, 2n], { account: alices[2] })
    ).to.be.rejectedWith("PermissionDenied");
  });
  ...
  it("Member can't add or remove admin", async function () {
    ...
    await expect(
      membership.write.setMember([uid, 3n, 4n, 0b0010111n], {
        account: alices[2],
      })
    ).to.be.rejectedWith("PermissionDenied");
    await expect(
      membership.write.setMember([uid, 3n, 4n, 0b0001111n], {
        account: alices[2],
      })
    ).to.be.rejectedWith("PermissionDenied");
    await membership.write.setMember([uid, 3n, 4n, 0b0000111n], {
    ...
    await expect(
      membership.write.setMember([uid, 4n, 5n, 0b0011111n], {
        account: alices[3],
      })
    ).to.be.rejectedWith("PermissionDenied");
    ...
    await expect(
      membership.write.setMember([uid, 4n, 5n, 0b1111111n], {
        account: alices[3],
      })
    ).to.be.rejectedWith("PermissionDenied");
    ...
    await expect(
      membership.write.removeMember([uid, 4n, 3n], { account: alices[3] })
    ).to.be.rejectedWith("PermissionDenied");

    // Member can't remove super admin
    await expect(
      membership.write.removeMember([uid, 4n, 2n], { account: alices[3] })
    ).to.be.rejectedWith("PermissionDenied");
  });
  ...
  it("Can always leave", async function () {
    ...
    await membership.write.setMember([uid, 2n, 3n, 0b1111011n], {
      account: alices[1],
    });
  ...
  it("Admins Can't Demote Super Admins", async function () {
    const { alices, membership, uid } = await loadFixture(deployFixture);
    ...
    // Admin shouldn't be able to reduce super admin permissions
    expect(await membership.read.getMember([uid, 2n])).to.deep.equal([
      true,
      0b1111111n,
    ]);
    await expect(
      membership.write.setMember([uid, 3n, 2n, 0b0000011n], {
        account: alices[2],
      })
    ).to.be.rejectedWith("PermissionDenied");
    expect(await membership.read.getMember([uid, 2n])).to.deep.equal([
    ...
    await expect(
      membership.write.removeMember([uid, 3n, 2n], {
        account: alices[2],
      })
    ).to.be.rejectedWith("PermissionDenied");
    expect(await membership.read.getMember([uid, 2n])).to.deep.equal([
  ...
  it("Admins can promote themselves to any membership-level permission", async function () {
    const { alices, membership, uid } = await loadFixture(deployFixture);
    ...
    await membership.write.setMember([uid, 2n, 3n, 0b0011100n], {
      account: alices[1],
    }); // Admin

    // Admin tries to promote himself with membership-level permissions (attest/revoke) added to his current permissions.
    await membership.write.setMember([uid, 3n, 3n, 0b0011111n], {
      account: alices[2],
    });
    expect(await membership.read.getMember([uid, 3n])).to.deep.equal([
      true,
    ...
    // Admin tries to promote himself with membership-level permissions (attest/revoke), resulting in loosing admin rights.
    await membership.write.setMember([uid, 3n, 3n, 0b0000111n], {
      account: alices[2],
    });
    expect(await membership.read.getMember([uid, 3n])).to.deep.equal([
      true,
