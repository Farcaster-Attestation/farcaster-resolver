...
const FarcasterResolverInteropModule = buildModule(
  "FarcasterResolverInteropModule",
  (m) => {
    const { resolver, ...rest } = m.useModule(FarcasterResolverModule);

    const interop = m.contract(
      "FarcasterResolverInterop",
      [resolver, m.getParameter("sourceChainId", 31337)],
      {}
    );
